...
try {
 $taskDirectory='deliverables/v0.0.42'
 ...
 $taskRemote=& git @taskArgs ls-remote origin refs/heads/main refs/tags/v0.0.42
 if ($LASTEXITCODE -ne 0) { throw 'Remote unavailable' }
 $taskHead=git rev-parse HEAD
 if (-not ($taskRemote -contains "$taskHead`trefs/heads/main") -or -not ($taskRemote -contains "$taskHead`trefs/tags/v0.0.42")) { throw 'Remote version mismatch' }
 $taskPointer=git show "v0.0.42:$($taskManifest.model)"
 if (-not ($taskPointer -contains "oid sha256:$taskHash")) { throw 'LFS pointer differs' }
 if (git diff --name-only v0.0.41 HEAD -- models/campus/WZMS_Campus_v041.blend deliverables/v0.0.41 web-preview reference/source reference/reports) { throw 'Frozen previous version or references changed' }
 git lfs fsck
 if ($LASTEXITCODE -ne 0) { throw 'LFS integrity check failed' }
 if (git status --porcelain) { throw 'Uncommitted changes remain' }
 [pscustomobject]@{Version='v0.0.42';Commit=$taskHead;ModelSHA256=$taskHash;ReviewedImages=4;Routes=74;Verified=$true} | ConvertTo-Json | Set-Content builds/final_south_layout_delivery_audit.json -Encoding utf8
 Write-Output 'v0.0.42: model, 4 images, 74 routes, south layout, relocated gate content, remote tag and LFS verified.'
}
