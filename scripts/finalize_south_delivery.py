"""Seal a reviewed southern milestone after its saved-file render and geometry audits.
Use only after actually viewing every image: python ... 12 --images-reviewed
"""
...
if not a.images_reviewed:raise SystemExit('Review the rendered images before sealing the delivery.')
ver=f'v0.0.{a.revision}';out=ROOT/'deliverables'/ver
...
for rec in r['renders']:
 ...
 pos=8;compressed=[]
 while pos<len(data):
  length=struct.unpack('>I',data[pos:pos+4])[0];typ=data[pos+4:pos+8];body=data[pos+8:pos+8+length]
  crc=struct.unpack('>I',data[pos+8+length:pos+12+length])[0];assert zlib.crc32(typ+body)&0xffffffff==crc
