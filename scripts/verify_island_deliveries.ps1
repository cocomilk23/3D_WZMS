...
try {
    ...
    $taskRows = foreach ($taskRevision in 22..25) {
    ...
    $taskSouthWall = Get-Content -LiteralPath 'deliverables/v0.0.25/south_gate_preservation_validation.json' -Raw | ConvertFrom-Json
    ...
    $taskRoutes = Get-Content -LiteralPath 'deliverables/v0.0.25/geometry_validation.json' -Raw | ConvertFrom-Json
    if (-not $taskRoutes.all_passed -or $taskRoutes.routes.Count -ne 22 -or ($taskRoutes.routes | Where-Object failure_count -ne 0)) { throw 'Integrated route check incomplete' }
    if (git diff --name-only 017cf06 HEAD -- web-preview reference/source reference/reports) { throw 'Frozen web or original references changed' }
    git lfs fsck
    if ($LASTEXITCODE -ne 0) { throw 'Git LFS integrity failed' }
    if (git status --porcelain) { throw 'Uncommitted workspace changes' }
    $taskRows | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $taskRoot 'builds/final_island_delivery_audit.json') -Encoding utf8
