...
try {
 $taskDirectory='deliverables/v0.0.41'
 ...
 if (-not $taskManifest.images_visually_reviewed -or $taskManifest.previews.Count -ne 3) { throw 'Review incomplete' }
 ...
 $taskExpectedX=@(74,92,110,128)
 if ($taskLayout.tennis_centres.Count -ne 4) { throw 'Four-court row incomplete' }
 for ($taskIndex=0; $taskIndex -lt 4; $taskIndex++) {
  if ($taskLayout.tennis_centres[$taskIndex][0] -ne $taskExpectedX[$taskIndex] -or $taskLayout.tennis_centres[$taskIndex][1] -ne 8) { throw 'Four-court row mismatch' }
 }
 ...
 $taskRemote=& git @taskArgs ls-remote origin refs/heads/main refs/tags/v0.0.41
 if ($LASTEXITCODE -ne 0) { throw 'Remote unavailable' }
 $taskHead=git rev-parse HEAD
 if (-not ($taskRemote -contains "$taskHead`trefs/heads/main") -or -not ($taskRemote -contains "$taskHead`trefs/tags/v0.0.41")) { throw 'Remote version mismatch' }
 $taskPointer=git show "v0.0.41:$($taskManifest.model)"
 if (-not ($taskPointer -contains "oid sha256:$taskHash")) { throw 'LFS pointer differs' }
 if (git diff --name-only v0.0.40 HEAD -- models/campus/WZMS_Campus_v040.blend deliverables/v0.0.40 web-preview reference/source reference/reports) { throw 'Frozen previous version or references changed' }
 git lfs fsck
 if ($LASTEXITCODE -ne 0) { throw 'LFS integrity check failed' }
 if (git status --porcelain) { throw 'Uncommitted changes remain' }
 [pscustomobject]@{Version='v0.0.41';Commit=$taskHead;ModelSHA256=$taskHash;ReviewedImages=3;Routes=68;Verified=$true} | ConvertTo-Json | Set-Content builds/final_tennis_orientation_delivery_audit.json -Encoding utf8
 Write-Output 'v0.0.41: model, 3 images, 68 routes, court orientation, deferred wall, remote tag and LFS verified.'
}
