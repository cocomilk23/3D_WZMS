...
args=sys.argv[sys.argv.index('--')+1:];version=args[0];names=[x for x in args[1:] if x not in ['--no-persistent-data','--cpu']]
...
scene.cycles.device='GPU' if any(d['use'] for d in devices) and '--cpu' not in args else 'CPU'
...
report={'opened_saved_blend':bpy.data.filepath,'version':scene.get('delivery_version'),
 ...
 'devices':devices,'render_device':scene.cycles.device,'persistent_render_data':bool(scene.render.use_persistent_data),'samples':scene.cycles.samples,'renders':r,'absolute_scale_verified':False,'user_acceptance':'pending','ue_playthrough_verified':False}
