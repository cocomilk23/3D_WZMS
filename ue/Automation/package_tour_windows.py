...
root=pathlib.Path(__file__).resolve().parents[2];ue=root/'ue';project=ue/'WZMS/WZMS.uproject';parser=argparse.ArgumentParser();parser.add_argument('--version',default='0.1.0-preview');args=parser.parse_args();assert re.fullmatch(r'[0-9A-Za-z._-]+',args.version)
...
for file,count in requirements.items():
 data=json.loads((ue/'Reports'/file).read_text(encoding='utf8'));assert data.get('passed'),file
 if count is not None:assert data.get('complete') and len(data.get('cases',data.get('tests',[])))==count,file
...
commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip();release=pathlib.Path('E:/WZMS_Releases')/args.version;release.mkdir(parents=True,exist_ok=True)
...
(release/'source-manifest.json').write_text(json.dumps({'source_commit':commit,'source_files_sha256':files},indent=2),encoding='utf8');state={'version':args.version,'source_commit':commit,'state':'running','started_unix':time.time(),'command':command,'log':str(release/'build.log')}
