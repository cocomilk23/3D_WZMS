"""Check source and later tour luminaires along their emission axis; not a full lighting test."""
import unreal,json
...
for a in unreal.GameplayStatics.get_all_actors_of_class(world,unreal.Actor):
 ...
 if not name.startswith(('InteriorLight_','Tour_InteriorLight_','Tour_InteriorBounce_')):continue
 ...
 extra={}
 for key in ['affects_world','visible','intensity_units','attenuation_radius','use_inverse_squared_falloff','lighting_channels','cast_shadows','max_draw_distance','max_distance_fade_range','temperature','use_temperature']:
  try:extra[key]=str(c.get_editor_property(key))
  except Exception:pass
 rows.append({'name':name,'component_class':c.get_class().get_name(),'position':list(pos.to_tuple()),'forward':list(direction.to_tuple()),'intensity':c.intensity,'color':str(c.get_light_color()),'properties':extra,'hit':result})
