...
try:
    for camera,filename in shots:
        filename+='.png';out=root/'Reviews/Campus'/filename;start=time.time()
        (root/'WZMS/Saved/Logs/review_request.json').write_text(json.dumps({'camera':'Review_'+camera,'filename':filename,'group':'Campus'}))
