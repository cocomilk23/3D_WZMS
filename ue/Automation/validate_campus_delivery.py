...
for zone in ['south','central','west','east','north']:
    ...
    for ref in imported['chunks']:
        ...
        assert mesh.get_num_triangles(0)==ref['ue_triangles'],ref['name']
        ...
        if ref['role']=='foliage':
            ...
            assert ns.keep_percent_triangles==1 and ns.fallback_percent_triangles==1
        for i,slot in enumerate(mesh.static_materials):
            mi=comp.get_material(i);assert mi and mi.get_name()=='MI_'+str(slot.material_slot_name),(ref['name'],i)
        rows.append({'zone':zone,'mesh':ref['name'],'triangles':ref['ue_triangles'],'transform_correct':True,'materials_bound':True,'collision_policy_verified':True})
