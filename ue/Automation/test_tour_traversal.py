...
output=root.parent/'Reports'/req.get('report','tour_stair_baseline.json');results={'complete':False,'cases':[],'method':'Actual CharacterMovement following connected waypoints; teleport only at each route start.'};state={};holder=[None]
...
def enter(i):
 ...
 state.update(case=i,waypoint=1,start=time.monotonic(),phase='settle',samples=[],last_sample=0,anchor=pawn.get_actor_location(),last_progress=time.monotonic())
