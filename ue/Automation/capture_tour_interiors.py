...
p=argparse.ArgumentParser();p.add_argument('--only',nargs='*');p.add_argument('--prefix',default='');args=p.parse_args()
if args.only:shots=[s for s in shots if s[1] in args.only]
client=UnrealMcpClient();client.connect()
try:
 for camera,name in shots:
  out=root/'Reviews/Tour'/(args.prefix+name+'.png');start=time.time()
