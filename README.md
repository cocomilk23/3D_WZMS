...
- [v0.0.33篮球场与西侧运动区整合](deliverables/v0.0.33/README.md) · **[最新整合Blender工程](models/campus/WZMS_Campus_v033.blend)**
- [Blender样板模型](models/south_gate/WZMS_SouthGate_v002.blend)（Git LFS，含打包的墙面参考图）

西门与运动区本轮交付记录见 v0.0.30—v0.0.33；每场景有独立工程、审查图和版本标签，各版本的检查结果以对应交付目录为准。当前工作区为 `E:\3D_WZMS`。

![西门与运动区接入后的校园初版](deliverables/v0.0.33/previews/138_Campus_integrated_v033.png)

首版校名和浮雕采用原照片表面细节，尚非独立雕刻网格；尺寸仍为估算，尚未导入UE。

每个可检查工作单元提交并推送，阶段完成后建立标签。模型资产使用Git LFS。
**全量高清全景素材库仍保存在本机 E 盘项目内；新克隆需要另行获取素材包才能使用完整图库。最新工程内已打包南门正反面、中山路牌、校友风采展板与操场草坪所用原图，查看已制作场景不需要另找这些贴图。**
...
2026-09-08：西门 v030、操场 v031、体育馆外景 v032、篮球场与运动区整合 v033 已独立交付。最新 [v034 篮球场修正](deliverables/v0.0.34/README.md) 按用户要求改为八块、4×2 排列，各场旋转 90°。下一版修正网球场，随后补齐有全景依据的本部室内。新疆部按用户要求暂停。逐场景 Git 管理，网页保持 v0.0.16（展示 v015）。

步青广场至北门保留 v0.0.7–v0.0.11；南侧四场景保留 v0.0.12–v0.0.15。东侧文化区分别保留 v0.0.17–v0.0.21，图书馆外景与内景分开交付。岛屿与横向连接为 v0.0.22—v0.0.25，西侧文化区与南田路为 v0.0.26—v0.0.29。下一步可按素材覆盖清单推进新疆部及有依据的建筑室内，随后统一校准与深化材质，并接入 UE 漫游。数学馆、校史馆、艺术楼、体育馆与食堂的完整内景、未拍摄的图书馆阅览室及完整籀园学堂室内尚未完成。后续顺序见[从南大门向北的制作顺序](docs/南北向制作顺序.md)。[用户反馈](docs/用户反馈与待调整.md)中的南门背面校歌问题按要求保留原状。
