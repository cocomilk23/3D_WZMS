...
v040 在现有本部内外景上进行整体构件与材质深化，并按复核后的航拍将四块网球场、球网和隔墙整体旋转 90°；荷屿与附近步道随之协调调整，具体推定范围见本版说明。
目标已确认：可自由行走的写实校园，兼顾航拍与地面细节。
当前用Blender组织可编辑内外景资产；UE漫游集成和运行时碰撞、性能验收尚未开始。
...
- [v0.0.40本部整体精修与网球场转向](deliverables/v0.0.40/README.md) · **[最新整合Blender工程](models/campus/WZMS_Campus_v040.blend)**
...
![整体构件与材质深化后的校园](deliverables/v0.0.40/previews/182_Refined_whole_campus.png)
...
步青广场至北门保留 v0.0.7–v0.0.11；南侧四场景保留 v0.0.12–v0.0.15。东侧文化区分别保留 v0.0.17–v0.0.21，图书馆外景与内景分开交付。岛屿与横向连接为 v0.0.22—v0.0.25，西侧文化区与南田路为 v0.0.26—v0.0.29。v040 为整体现有构件和材质的一轮深化；下一步按实际验收反馈校正，补充尺度依据，再接入 UE 漫游。现有室内仅覆盖可见区域及明确记录的连接推定；未拍摄的阅览室、厨房、更衣室、卫生间、其他楼层以及完整艺术楼、籀园学堂室内尚未完成。[用户反馈](docs/用户反馈与待调整.md)中的南门背面校歌问题按要求保留原状。
