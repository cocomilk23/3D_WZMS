...
已制作南大门样板，以及向校内连续延伸的南大门广场、德涵楼与贯真楼外景、中山路南段及跨水桥、步青广场、中山路中段和北段、食堂外景初版。全校园尚未建成，各版待用户验收。
...
- [v0.0.10食堂外景](deliverables/v0.0.10/交付说明.md) · **[最新整合Blender工程](models/campus/WZMS_Campus_v010.blend)**
- [Blender样板模型](models/south_gate/WZMS_SouthGate_v002.blend)（Git LFS，含打包的墙面参考图）

![食堂外景初版](deliverables/v0.0.10/previews/34_Canteen_long_front.png)
...
最新路线见[从南大门向北的制作顺序](docs/南北向制作顺序.md)。南侧三场景之后，步青广场 v0.0.7 和中山路中段 v0.0.8 初版已制作；中山路北段 v0.0.9 也已制作；食堂外景 v0.0.10 已制作；下一场景为北门。每完成一个场景独立提交、推送并建立版本标签，不再更新网页预览。照片依据及估算边界见[南侧建模依据](docs/南侧三场景建模依据.md)和[步青广场至北门依据](docs/步青广场至北门建模依据.md)。[用户反馈](docs/用户反馈与待调整.md)中记录的南门背面校歌问题按要求暂不修改。

1. 按建筑制作覆盖与细节清单，区分可见、有依据推断、完全未知区域。
2. 建立尺度和坐标依据，保留误差；视角角度不能充当测量坐标。
3. 从步青广场继续向北，并逐场景补齐两侧建筑、道路、水系与运动场。
