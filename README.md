...
已制作南大门样板，以及沿南门广场—德涵楼/贯真楼—跨水桥—步青广场—中山路中段/北段—北门的连续外景初版，并接入食堂外景。保留网球场、道司前路、荷屿和水南路四个南侧场景；本批加入图书馆外景与有素材的一、二层大厅、中庭和可见楼梯、数学馆外景、九山路与两座桥、校史馆外景。各版待用户验收，完整校园尚未建成。
...
- [v0.0.21校史馆外景与文化区整合](deliverables/v0.0.21/README.md) · **[最新整合Blender工程](models/campus/WZMS_Campus_v021.blend)**
- [Blender样板模型](models/south_gate/WZMS_SouthGate_v002.blend)（Git LFS，含打包的墙面参考图）

![东侧文化区整合初版](deliverables/v0.0.21/previews/84_Cultural_precinct_overview.png)
...
步青广场至北门保留 v0.0.7–v0.0.11；南侧四场景保留 v0.0.12–v0.0.15。本批文化区分别保留 v0.0.17–v0.0.21，图书馆外景与内景分开交付，其余为外景初版。数学馆与校史馆的展厅、未拍摄的图书馆阅览室及其余岛屿尚未完成。后续顺序见[从南大门向北的制作顺序](docs/南北向制作顺序.md)，照片依据见[东侧文化区建模依据](docs/东侧文化区建模依据.md)。[用户反馈](docs/用户反馈与待调整.md)中的南门背面校歌问题按要求保留原状。
